...
import { IAddressPostalCode, IUser } from '@/@core/@types/interface';
...
const AddressPostalCodePageTable = () => {
  ...
  const exportData = async () => {
    try {
      ...
      const rows: IAddressPostalCode[] = resp.data?.results || [];
      ...
      const worksheet = workbook.addWorksheet('Data Postal Code');
      ...
      saveAs(
        new Blob([buffer]),
        `laporan_data_postal_code_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
      );
    } catch (error) {
      console.error('Export data postal code failed:', error);
    } finally {
