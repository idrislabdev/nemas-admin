...
const PaymentMethodPageTable = () => {
    ...
    const columns: ColumnsType<IPaymentMethod>  = [
        ...
        { title: 'Nama', dataIndex: 'payment_method_name', key: 'payment_method_name'},
        { title: 'Deskripsi', dataIndex: 'payment_method_description', key: 'payment_method_description', width: 150},
        ...
        { title: '', key: 'action', fixed: 'right', width:100,
          ...
          (<div className='flex items-center gap-[5px] justify-center'>
            <Link href={`/payment/payment-method/${record.payment_method_id}`} className="btn-action"><Edit05 /></Link>
            <a className='btn-action' onClick={() => deleteData(record.payment_method_id)}><Trash01 /></a>
        </div>)
        },
    ];
    ...
    const exportData = async () => {
        ...
        const colA = 5;
        const colB = 10;
        const colC = rows.reduce((w:number, r:IPaymentMethod) => Math.max(w, r.payment_method_name ? r.payment_method_name.length : 10), 10);
        const colD = rows.reduce((w:number, r:IPaymentMethod) => Math.max(w, r.payment_method_description ? r.payment_method_description.length : 10), 10);
    ...
    return (
        ...
        <div className='flex items-center justify-between'>
            ...
            <div className='flex items-center gap-[4px]'>
                ...
                <Link href={`/payment/payment-method/form`} className="btn btn-outline-neutral"><Plus />Add data</Link>
            </div>
        </div>
