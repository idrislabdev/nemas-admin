...
const GoldCertDetailTable = (props: {goldId:string, certificateId:string}) => {
    ...
    const columns: ColumnsType<IGoldCertPriceDetail>  = [
        ...
        { title: 'Kode Sertifikat', dataIndex: 'gold_cert_code', key: 'gold_cert_code', width:150},
        { title: 'Satuan (gr)', dataIndex: 'gold_weight', key: 'gold_weight', width:120,
            render: (_, record) => (`${formatterNumber(record.gold_weight ? record.gold_weight : 0)} gr`) 
        },
        { title: 'Include Stock', dataIndex: 'include_stock', key: 'include_stock', width:150,
            render: (_, record) => (`${record.include_stock ? 'Ya' : 'Tidak'}`) 
        },
        { title: '', key: 'action', fixed: 'right', width:100,
          render: (_, record) =>
          (<div className='flex items-center gap-[5px] justify-center'>
            <a className="btn-action" onClick={() => editData(record.id ? record.id.toString() : '')}><Edit05 /></a>
            <a className='btn-action' onClick={() => deleteData(record.id)}><Trash01 /></a>
        </div>)
        },
    ];
    ...
    return (
       <>
            ...
            <div className='flex items-center justify-end'>
                <a className="btn btn-outline-neutral cursor-pointer" onClick={() => addNewData()}><Plus />Tambah Sertifikat</a>
            </div>
