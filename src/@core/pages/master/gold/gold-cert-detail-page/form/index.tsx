...
const GoldCertDetailPageForm = (props: {paramsId:string}) => {
    ...
    const [required, setRequired] = useState<IGoldCertPriceDetail>({} as IGoldCertPriceDetail);
    ...
    <div className='form-input'>
        ...
        <div className='form-area'>
            <div className='input-area'>
                <label>Emas {required.gold && <span className='text-red-500 text-[10px]/[14px] italic'>({required.gold?.toString()})</span>}</label>
                <select 
                    defaultValue={gold} 
                    onChange={e => setGold(parseInt(e.target.value))}
                >
                    {golds.map((item, index:number) => (
                        <option value={item.gold_id} key={index}>{item.brand} - {item.gold_weight}Gr</option>
                    ))}
                </select>
            </div>
            <div className='input-area'>
                <label>Sertifikat {required.gold_cert && <span className='text-red-500 text-[10px]/[14px] italic'>({required.gold_cert?.toString()})</span>}</label>
                <select 
            ...
            <div className='input-area'>
                <label>Kode Sertifikat {required.gold_cert_code && <span className='text-red-500 text-[10px]/[14px] italic'>({required.gold_cert_code?.toString()})</span>}</label>
                <input 
                    value={goldCertCode} 
            ...
            <div className='input-area'>
                <label>Include Stock {required.include_stock && <span className='text-red-500 text-[10px]/[14px] italic'>({required.include_stock?.toString()})</span>}</label>
                <select 
                    defaultValue={includeStock ? 'Ya' : 'Tidak'} 
                    onChange={e => setIncludeStock(e.target.value == 'Ya' ? true : false)}
                >
