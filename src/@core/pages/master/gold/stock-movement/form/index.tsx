...
const GoldStockMovementPageForm = () => {
    ...
    const [required, setRequired] = useState<IGoldStockMovement>({} as IGoldStockMovement);
        ...
        <>
            <div className='form-input'>
                ...
                <div className="flex gap-[4px] items-center justify-end">
                      ...
                      <Link href={`/master/gold/stock-movement`} className="btn btn-outline-neutral"><FlipBackward /> Kembali</Link>
                  </div>
                <div className='form-area'>
                    <div className='input-area'>
                        <label>Berat Emas (gr) {required.weight && <span className='text-red-500 text-[10px]/[14px] italic'>({required.weight?.toString()})</span>}</label>
                    ...
                    <div className='input-area'>
                        <label>Tipe {required.transaction_type && <span className='text-red-500 text-[10px]/[14px] italic'>({required.transaction_type?.toString()})</span>}</label>
                        <select className={`base ${required.transaction_type ? 'error' : ''}`} defaultValue={transactionType} onChange={e => setTransactionType(e.target.value)} >
                            <option value={'IN'}>IN</option>
                            <option value={'OUT'}>OUT</option>
                        </select>
                    </div>
                    <div className='flex flex-col gap-[4px]'>
                        <label>Catatan {required.note && <span className='text-red-500 text-[10px]/[14px] italic'>({required.note?.toString()})</span>}</label>
                        <input value={note} onChange={e => setNote(e.target.value)} className='base' />
                    </div>
                </div>
            </div>
            <ModalLoading 
