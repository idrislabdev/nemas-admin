...
const GoldPricePageForm = (props: { paramsId: string }) => {
  ...
  const onSave = async () => {
    const body = {
      ...
      gold_price_weight: parseFloat(
        goldPriceWeight.toString().replace('.', '').replace(',', '.')
      ),
      gold_price_base: parseFloat(
        goldPriceBase.toString().replace('.', '').replace(',', '.')
      ),
      gold_price_sell: parseFloat(
        goldPriceSell.toString().replace('.', '').replace(',', '.')
      ),
      gold_price_buy: parseFloat(
        goldPriceBuy.toString().replace('.', '').replace(',', '.')
      ),
    };
    setRequired({});
    try {
    ...
    <div className="form-input">
      ...
      <div className="form-area">
        <div className="input-area">
          <label>
            Asal Harga Emas{' '}
            {required.gold_price_source && (
              <span className="text-red-500 text-[10px]/[14px] italic">
                ({required.gold_price_source?.toString()})
              </span>
            )}
          </label>
          <input
        ...
        <div className="input-area">
          <label>
            Satuan (gr){' '}
            {required.gold_price_weight && (
              <span className="text-red-500 text-[10px]/[14px] italic">
                ({required.gold_price_weight?.toString()})
              </span>
            )}
          </label>
        ...
        <div className="input-area">
          <label>
            Harga Dasar{' '}
            {required.gold_price_base && (
              <span className="text-red-500 text-[10px]/[14px] italic">
                ({required.gold_price_base?.toString()})
              </span>
            )}
          </label>
          <CurrencyInput
        ...
        <div className="input-area">
          <label>
            Harga Jual{' '}
            {required.gold_price_sell && (
              <span className="text-red-500 text-[10px]/[14px] italic">
                ({required.gold_price_sell?.toString()})
              </span>
            )}
          </label>
          <CurrencyInput
        ...
        <div className="input-area">
          <label>
            Harga Beli{' '}
            {required.gold_price_buy && (
              <span className="text-red-500 text-[10px]/[14px] italic">
                ({required.gold_price_buy?.toString()})
              </span>
            )}
          </label>
          <CurrencyInput
