    ...
    const onSave = async () => {
        ...
        try {
            ...
            if (paramsId == 'form') {
                ...
                const { data } = resp
                if (fileData != null)
                    await uploadFile(data.promo_id)
             ...
             <div className='form-input'>
                <div className='flex items-start gap-[10px]'>
                    ...
                    <div className='input-area'>
                            <label>Gambar / Background  {required.promo_url_background && <span className='text-red-500 text-[10px]/[14px] italic'>({required.promo_url_background?.toString()})</span>}</label>
                            <UploadForm 
                                index={1}
                                withFile={false} 
                                label='' 
                        ...
                        <div className='input-area'>
                            <label>Kode Promo  {required.promo_code && <span className='text-red-500 text-[10px]/[14px] italic'>({required.promo_code?.toString()})</span>}</label>
                            <input value={promoCode} onChange={e => setPromoCode(e.target.value)} className='base' />
                        </div>
                        <div className='input-area'>
                            <label>Level User  {required.leveling_user && <span className='text-red-500 text-[10px]/[14px] italic'>({required.leveling_user?.toString()})</span>}</label>
                            <input value={levelingUser} onChange={e => setLevelingUser(e.target.value)} className='base' />
                        </div>
                        <div className='input-area'>
                            <label>Nama Promo  {required.promo_name && <span className='text-red-500 text-[10px]/[14px] italic'>({required.promo_name?.toString()})</span>}</label>
                            <input value={promoName} onChange={e => setPromoName(e.target.value)} className='base' />
                        </div>
                        <div className='input-area'>
                            <label>Promo URL  {required.promo_url && <span className='text-red-500 text-[10px]/[14px] italic'>({required.promo_url?.toString()})</span>}</label>
                    ...
                    <div className='form-area w-1/2'>
                        <div className='input-area'>
                            <label>Tanggal Mulai Promo  {required.promo_start_date && <span className='text-red-500 text-[10px]/[14px] italic'>({required.promo_start_date?.toString()})</span>}</label>
                            <input value={promoStartDate} onChange={e => setPromoStartDate(e.target.value)} type='date' className='base' />
                        </div>
                        <div className='input-area'>
                            <label>Tanggal Berakhir Promo  {required.promo_end_date && <span className='text-red-500 text-[10px]/[14px] italic'>({required.promo_end_date?.toString()})</span>}</label>
                            <input value={promoEndDate} onChange={e => setPromoEndDate(e.target.value)} type='date' className='base' />
                        </div>
                        <div className='input-area'>
                            <label>Promo Tag  {required.promo_tag && <span className='text-red-500 text-[10px]/[14px] italic'>({required.promo_tag?.toString()})</span>}</label>
                            <input value={promoTag} onChange={e => setPromoTag(e.target.value)} type='text' className='base' />
                        </div>
                        <div className='input-area'>
                            <label>Diskon  {required.promo_diskon && <span className='text-red-500 text-[10px]/[14px] italic'>({required.promo_diskon?.toString()})</span>}</label>
                            <CurrencyInput
                                value={promoDiskon}
                                decimalsLimit={2}
                        ...
                        <div className='input-area'>
                            <label>Promo Cashback  {required.promo_cashback && <span className='text-red-500 text-[10px]/[14px] italic'>({required.promo_cashback?.toString()})</span>}</label>
                            <CurrencyInput
                                value={promoCashback}
                                decimalsLimit={2}
                                decimalSeparator="," groupSeparator="." 
                        ...
                        <div className='input-area'>
                            <label>Merchant Cashback  {required.merchant_cashback && <span className='text-red-500 text-[10px]/[14px] italic'>({required.merchant_cashback?.toString()})</span>}</label>
                            <CurrencyInput
                                value={merchantCashback}
                                decimalsLimit={2}
                                decimalSeparator="," groupSeparator="." 
                                onValueChange={(value) => setMerchantCasbhack(value ? value : "0")}
                                className='base'
                                placeholder='0'
                            />
                        </div>
