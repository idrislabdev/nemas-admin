...
import { IAddressProvince, IUser } from '@/@core/@types/interface';
...
const AddressProvincePageTable = () => {
  ...
  const exportData = async () => {
    try {
      ...
      const rows: IAddressProvince[] = resp.data?.results || [];

      if (!rows.length) return;

      const dataToExport = rows.map(
        (item: IAddressProvince, index: number) => ({
          No: index + 1,
      ...
      const worksheet = workbook.addWorksheet('Data Province');

      const lastColumnLetter = 'B';

      /* ================= TITLE ================= */
      worksheet.mergeCells(`A1:${lastColumnLetter}1`);
      worksheet.getCell('A1').value = 'LAPORAN DATA PROVINCE';
      ...
      saveAs(
        new Blob([buffer]),
        `laporan_data_province_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
      );
    } catch (error) {
      console.error('Export data province failed:', error);
    } finally {
...
export default AddressProvincePageTable;
