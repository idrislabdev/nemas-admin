            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Description</th>
                        <th>Price Weight</th>
                        <th>Price Buy Weekday</th>
                        <th>Price Sell Weekday</th>
                        <th>Price Buy Weekend</th>
                        <th>Price Sell Weekend</th>
                        <th>Status</th>
                        <th>Created By</th>
                        <th>Updated By</th>
                ...
                <tbody>
                    {
                        dataTable.map((item:IGoldPriceConfig, index:number) => (
                            <tr key={index}>
                                <td>{index+1}</td>
                                <td>{item.gpc_code}</td>
                                <td>{item.gpc_description}</td>
                                <td>{item.gold_price_weight}</td>
                                <td>{item.gold_price_setting_model_buy_weekday}</td>
                                <td>{item.gold_price_setting_model_sell_weekday}</td>
                                <td>{item.gold_price_setting_model_buy_weekend}</td>
                                <td>{item.gold_price_setting_model_sell_weekend}</td>
                                <td>{item.gpc_active ? 'Active' : 'Not Active'}</td>
                                <td>{item.create_user}</td>
                                <td>{item.upd_user}</td>
                                <td className='text-center'>
                                    <div className='flex items-center gap-[5px] justify-center'>
                                        <a className='btn-action' onClick={() => deleteData(item.gpc_id)}><TrashOutlineIcon /></a>
