            ...
            <div className='form-input'>
                <div className='form-area'>
                    <div className='input-area'>
                        <label>Nama Informasi {required.information_rate_name && <span className='text-red-500 text-[10px]/[14px] italic'>({required.information_rate_name?.toString()})</span>}</label>
                        <input value={informationRateName} onChange={e => setInformationRateName(e.target.value)} className='base' />
                    </div>
                    <div className='input-area'>
                        <label>Rate {required.rate && <span className='text-red-500 text-[10px]/[14px] italic'>({required.rate?.toString()})</span>}</label>
                        <CurrencyInput
                            value={rate}
                            decimalsLimit={2}
                    ...
                    <div className='input-area'>
                        <label>Message / Pesan {required.message && <span className='text-red-500 text-[10px]/[14px] italic'>({required.message?.toString()})</span>}</label>
                        <textarea value={message} onChange={e => setMessage(e.target.value)} className='base' />
                    </div>
                    <div className='input-area'>
                        <label>Publish {required.publish && <span className='text-red-500 text-[10px]/[14px] italic'>({required.publish?.toString()})</span>}</label>
